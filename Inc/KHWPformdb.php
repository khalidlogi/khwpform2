...
class KHdb {
	...
	public function count_items($formid) {
		
		global $wpdb;
		$items_count = 0;
		
		$items_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(DISTINCT id) FROM $this->table_name where form_id = $formid" ) );
    ...
    public function retrieve_id()
    {
        ...
        $results = $wpdb->get_results("SELECT form_id FROM $table_name");
